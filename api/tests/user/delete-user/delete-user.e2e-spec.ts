...
import { UserResponse } from '@src/modules/efrequser/interface/dtos/user.response.dto';
...
defineFeature(feature, test => {
  ...
  test('Deleting a user happy path', ({ given, when, then, and }) => {
    ...
    given(
      /^that my email is "(.*), my city is "(.*)", my postal code is "(.*)" and my street is "(.*)"$/,
      (email: string, city: string, postalCode: string, street: string) => {
        userDto = {
          email,
          city,
          postalCode,
